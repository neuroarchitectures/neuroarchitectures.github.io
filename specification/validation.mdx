...
- Components without a `scope` **when scope usage is inconsistent within the document** — i.e. some components carry a `scope` while others do not. A document that omits `scope` everywhere is consistent (scope is optional metadata, [§12](/specification/graph-model#12-scope-notation)) and produces no warning.
